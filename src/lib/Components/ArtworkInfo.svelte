...
{#if $sceneStore.startsWith('artwork')}
    <div in:fade={{ delay: 2000 , duration: 1000 }} on:click|stopPropagation class="select-none cursor-default absolute left-4 bottom-4 space-y-2 ">
        <div class="relative flex items-center justify-between">
            ...
            <img draggable="false" class="absolute left-1/2 -translate-x-1/2 -bottom-4" src={`${lastValidScene}-icon.png`} alt="scene icon">
        </div>

        <div class="relative">
            <img draggable="false" src="/artwork-info.png" alt="artwork info">
            ...
            {:else}
                <div class="absolute left-12 top-1/2 -translate-y-1/2 space-y-3 w-2/3">
                    <h1 class="text-white text-2xl font-semibold font-garamond uppercase tracking-wide">Artist Name</h1>
                    <p class="text-white text-xs font-monda tracking-tight">The Island of Fire offers an escape through intensity and passion.</p>
                </div>
            {/if}
        </div>
