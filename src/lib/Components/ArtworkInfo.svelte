...
{#if $sceneStore.startsWith('artwork')}
    <div in:fade={{ delay: 2000 , duration: 1000 }} on:click|stopPropagation class="select-none cursor-default absolute left-1/2 -translate-x-1/2 md:left-4 md:-translate-x-0 bottom-32 md:bottom-4 space-y-3 md:space-y-2 w-full md:w-auto px-7 md:px-0">
        <div class="relative flex items-center justify-between">
            ...
            <img draggable="false" class="absolute left-1/2 -translate-x-1/2 -bottom-5 md:-bottom-4" src={`${lastValidScene}-icon.png`} alt="scene icon">
        </div>

        <div class="relative">
            <img draggable="false" class="hidden md:block" src="/artwork-info.png" alt="artwork info">
            <img draggable="false" class="md:hidden w-full" src="/artwork-info-mobile.png" alt="artwork info">
            {#if isArtworkSelected}
                <div class="absolute left-4 md:left-12 top-1/2 -translate-y-1/2 flex md:flex-col md:gap-3 md:w-2/3">
                    <h1 class="text-white text-sm md:text-2xl font-semibold font-garamond uppercase tracking-wide">Eye faces character</h1>
                    <p class="text-white text-xs font-monda tracking-tight font">The Island of Fire offers an escape through intensity and passion.</p>
                </div>
            {:else}
                <div class="absolute left-4 md:left-12 top-1/2 -translate-y-1/2 flex md:flex-col gap-3 md:w-2/3">
                    <h1 class="text-white text-sm md:text-2xl font-semibold font-garamond uppercase tracking-wide">Artist Name</h1>
                    <p class="text-white text-xs font-monda tracking-tight font">The Island of Fire offers an escape through intensity and passion.</p>
                </div>
            {/if}
        </div>
