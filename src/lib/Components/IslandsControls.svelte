...
<div in:fade={{ delay: 2000 , duration: 1000 }} class="absolute left-1/2 -translate-x-1/2 md:translate-x-0 md:left-8 bottom-4 md:top-8 flex flex-col gap-3 items-center drop-shadow-cloud w-full md:w-auto h-fit">
  
  {#if $sceneStore.includes('artwork')}
    <button  on:click|stopPropagation={setScene.bind(null, lastValidScene)} class="{ $sceneStore === 'idle' ? '' : 'opacity-75' } absolute bottom-0 left-4 md:relative md:left-0 self-start uppercase font-semibold font-garamond tracking-wide border-y-[1px] border-white text-white py-1">return</button>
  {:else}
    <div class="flex items-center justify-between gap-4">
      <button on:click|stopPropagation={setScene.bind(null, 'idle')} class="{ $sceneStore === 'idle' ? '' : 'opacity-25' } absolute left-2 md:relative md:inset-0"><img src="/go-back.png" alt="return">
      </button>

      {#each scenes as scene}
        <button class="{ lastValidScene === scene ? '' : 'invert opacity-50' } flex items-center justify-center" on:click|stopPropagation={setScene.bind(null, scene)}>
  ...
  <div transition:fade={{ delay: 500 , duration: 500 }} class="hidden md:block font-garamond w-full font-semibold text-2xl text-white whitespace-nowrap uppercase">
