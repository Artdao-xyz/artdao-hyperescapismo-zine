...
export default class IslandFire {
	constructor() {
		...
		if (this.debug.active) {
			...
			positions.islandFire.artwork1.position
			...
			positions.islandFire.artwork2.position
			...
			positions.islandFire.artwork3.position
			...
			positions.islandFire.artwork4.position
			...
			positions.islandFire.artwork5.position
	...
	setModel() {
		this.model.position.set(
			positions.islandFire.position.x,
			positions.islandFire.position.y,
			positions.islandFire.position.z
		);
	...
	setMaterial() {
		...
		this.model.traverse((child) => {
			if (
				(child.isMesh && child.name === 'island-fire') ||
				(child.isMesh && child.name === 'portal-fire')
			) {
	...
	addArtworks() {
		...
		this.artworks.push(
			artwork1.artworkMesh,
			artwork2.artworkMesh,
			artwork3.artworkMesh,
			artwork4.artworkMesh,
			artwork5.artworkMesh
		);
